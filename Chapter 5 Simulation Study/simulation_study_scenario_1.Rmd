...
knitr::opts_chunk$set(fig.path = "README_figs/README-", echo = TRUE, message = FALSE, warning = FALSE, results = FALSE, fig.show = TRUE)
library(mgcv)
require(ROCnReg)
```

This document presents a simulation study where we compare the performance of the P-splines method for estimating the covariate-specific ROC curve against that of an estimator based on kernel methods to estimate mean and variance functions. The references for the kernel approach are Pardo-Fernandez et al. (2011, Scandinavian Journal of Statistics) and Rodriguez-Alvarez et al. (2011, Statistics and Computing). This kernel approach is available in the $\texttt{cROC.kernel}$ of the $\texttt{ROCnReg}$ package.
...
$$
The covariates, $x_{\bar{D}_i}$ and $x_{D_j}$, are assumed to be independently and uniformly distributed on the $(-1,\,1)$ interval. For simplicity, we further assume that errors are normally distributed, that is, $\varepsilon_{\bar{D}_i}$, $\varepsilon_{D_j} \sim \mathcal{N}(0,\,1)$ for $i = 1, \dots, n_{\bar{D}}$ and $j = 1, \dots, n_D$.
As sample sizes for the simulated data, we will consider $(n_D, n_{\bar{D}} ) = (100,\,100)$, $(n_D, n_{\bar{D}} ) = (200,\,200)$, and $(n_D, n_{\bar{D}} ) = (500,\,500)$. 
